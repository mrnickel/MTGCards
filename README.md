...
5. **Transfer between devices**: Collection → **Share QR**. The collection (set/collector-number/qty) is deflate-compressed and offered two ways. **Link code** is a single QR holding a `#import=…` URL — point the other phone's *normal camera* at it and the app opens and imports (works up to roughly 350 cards). **Parts** shows small codes that cycle automatically for the in-app **Scan QR** button, which works on any phone including iOS. **Copy link** shares the same URL by message or AirDrop. Scanning uses the native `BarcodeDetector` where available and falls back to jsQR. Cards are rehydrated from Scryfall and merged into that device's collection.
